...
import UserValidation from './middleware/user-validation.middleware.ts';
...
class UserRouter implements IUserRouter {
  ...
  injecting(): Router {
    this.router.get(`${this.baseAuthUrl}:id/statistics`, checkSchema(UserValidation.validationUserParamsId), validationError, UserController.getUserStatistics);
    return this.router;
  }
}
