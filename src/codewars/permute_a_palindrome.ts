...
export const permute_palindrome = (): string => {
  ...
  const word: string = !insert ? 'madam' : insert;

  console.log(chalk.green(`Word is: ${word}`));
  ...
  return check_palindrome(word) ? chalk.green.bold(`\n${insert} is polyndrom\n`) : chalk.red.bold(`\n${insert} is not a polydrome\n`);
};
